...
| --------------- | ------------------------------------------------- | -------------- | ------------------------- | ------------ | ------------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 0.5             | Инициация упрощенная без авторизации и регистрации посредством соц.сетей, тестирование                           | 14 (20)        | Nope                      | Done         |                                 |                                 | Требует доработки дизайна экранов входа, экранов просмотра процесса чат-бот режима, экранов регистрации. |
...
| 1.0             | Расширенная инициация. Расширенный трекинг питания и активности. Расширенный профиль. Каталог знаний. Генерация сторис                  | 3              | Done                      | Progress     |                                 |                                 |                                                                                                          |
...
>Анимация для квадратов: они в режиме карусели меняются по очереди, фокус остаётся на том квадрате, который в центре

![](/i/ui/svg/1.2.hello.svg)
>Анимация для деревьев: по ним просто пробегает блик
...
![](/i/ui/svg/20.chat_botGreeting.svg)
> Интерфейс полностью пройденного демо-процесса заполнения анкеты пользователя при регистрации при выборе «регистрация через Email».
...
Ветвления по сценарию возможны в случаях, когда пользователь решил авторизоваться через социальные сети.
В случае регистрации через любую социальную сеть, порядок вопросов/ответов перестраивается в зависимости от получения данных от соц.сети. Заполненные данные перемещаются наверх, после приветствия.

> Длительность индикации анимации процесса набора текста со стороны ЗОЖ-бота, должна быть равноценной объему отправленного сообщения. Это ключевой момент в технике работы чатбота для всех экранов связанных с анкетированием в этом.


## 4. Экраны навигации и прохождения полного анкетирования

Процедура интерактивного анкетирования пользователя — один из основных функциональных блоков после базовой регистрации пользователя.

Делится на 3 крупных раздела: 
...
Каждый блок чатбота вызывается при переходе пользователя в определённые разделы приложения после первичной установки. 

Самый популярный способ для пользователя, при вызове центральной кнопки синего цвета с плюсом на Таббаре и попытке добавить продукт/режим/активность в течении дня. 
...
Пользователь может в любой остановить  процесс прохождения тестирования в любом из чатботов. В Android через кнопку Back, и верхнюю навигацию. В iOS через верхнюю навигацию и свайп слева-на-право.
При этом все данные внесённые в процессе прохождения чатбота должны быть сохранены. И при повторном обращении в чат бот, происходит *auto-scroll* к месту последнего вопроса.


Полные данные о скрипте чат-бота представлены в [документе](Chat_script.pdf):

​```pdf
	Chat_script.pdf
​```

> Основной экран приложения со сквозной навигацией

![](/i/ui/svg/3.dashboard.svg)

> Всплывающий «зонтик» для возможности вызова трекеров данных пользователя

![](/i/ui/5.added_button_simple.png)

### Тестирование по режиму дня и образу жизни
>Прохождение анкеты с образом жизни и режима при вызове кнопки Режим из зонта навигации

![](/i/ui/svg/22.chat_botLife.svg)

### Медицинское и функциональное тестирование для мужчин и женщин

>Представление процесса медицинского теста с функциональным тестом для мужчин, при вызове кнопки Активность из зонта навигации

![](/i/ui/23.chat_bot_mode_all_screensMedical.png)

>Представление процесса функционального тестирования для женщин при вызове кнопки Режим из зонта навигации

![](/i/ui/7.fuctional_test_forWomen.png)


### Тестирование по питанию

>Представление процесса теста по питанию, при вызове кнопки Питание из зонта навигации

![](/i/ui/svg/21.chat_botNutrilon.svg)


Если пользователь не заполнил все персональные данные, то тренировки у него не могут быть просчитаны, графики тренированности и мезоциклов тоже. Поэтому ему предлагается продолжить чат с ЗОЖ

![](/i/ui/12.profile_blur.png)
...
## 5. Сайд-бар вызова вертикального меню пользователя

Сайдбар даёт дополнительную навигацию по разделам, которые недоступны из нижнего таббара, с менее популярными разделами.

![](i/ui/svg/4.sidebar.svg)

## 5.1 Раздел «Избранное»

Экран выполняет роль копилки для тех материалов, которые пользователь считает более приоритетными для себя. Соритровка отображения по дате добавления от последних к первым. 

![](/i/ui/svg/16.my_favorite.svg)
 

## 5.2 Экраны статистики

Экран содержит в себе 3 таба навигации. Переключение между которыми происходит как через директивный тап, так и через свайпы-вправо влево, с зацикливанием. Таким образом находясь на табе *Движение*, свайп справа-налево переключает на таб *Режим*.


Статистика о сне, самая краткая из всех остальных разделов. Пользователю доступны краткие рекомендации и напутствия. 
Механизм отображения циферблата строится по окружности с *вершиной к полуночи*. 
Если пользователь спал в нетипичное время, к примеру в промежутке с 11.00 до 17.00, то приложение *инвертирует циферблат* к веришине на 12.00.

![](/i/ui/svg/11.stats_of_day_sleep.svg)


Статистика для режима питания выражается в достижения заполнения колец по ключевым макром-микроэлементам. Белки, жиры и углеводы, вода.
Детализация каждого раздела, схлопывается через аккардеон.

Индикация воды заполняется через заливку окружности. В случае если пользователь превышает норму полной окружности, она начинает второй контур, с затемнением на 10% от начального синего цвета, во внешнюю часть окружности.

Калорийность, жиры, белки, вода, пищевые волокна, углеводы — используют для отображения *абсолютные значения*.
Каждый прогрессбар содержит в себе кроме шкалы заполнения — допустимый диапазон для этих элементов. Они отражаются вертикальными светлыми чертами на шкале.

Витамины и микроэлементы — *в процентном соотношении* от дневной нормы.
У витаминов и микроэлементов нет допустимых диапазонов, так как каждый человек должен употреблять не менее 100% указанных микроэлементов. 


![](/i/ui/svg/11.stats_of_day_food.svg)

Статистика физической активности пользователя заполняется по 3 основным показателям: моцион, тренировка, доп.активность. При этом если в просматриваемом дне не должно быть тренировки по графику тренировок заданному пользователем во время заполнения чат-бота, то окружность тренировки не отображается в диаграмме. 

В нижней части перечень с абсолютными значениями тех занятий, которые пользователь выбрал в каталоге тренировок.
Если пользователь дал доступ на считывание данных из внешних источников, таких как Apple Health, Google Fit и других провайдеров смарт-техники, то ходьба будет появляться автоматически для любого из просматриваемых дней, с момента получения приложением доступа к провайдерам.

![](/i/ui/svg/11.stats_of_day_sport.svg)


Пользователь может выбрать период просмотра статистики за период или за опредёленный день с помощью контрола выбора даты из системного для ОС.

![](/i/ui/svg/11.stats_of_day_period.svg)


Статистика на период содержит в себе календарную сетку с радиальными индикаторами качества дня. Если все круги заполнены — значит день прожит в полной мере.

![](/i/ui/svg/13.stats_general_1.svg)
